...
class GameShell {
  ...
  _updateWorld(silent) {
    ...
    if (this.cwd !== "/") {
      ...
      b.onclick = () => { this.inputEl.value = "cd .."; this._run(); };
    ...
    (loc.dirs||[]).forEach(d => {
      ...
      b.onclick = () => { this.inputEl.value = "cd " + d; this._run(); };
      this.exitsEl.appendChild(b);
    });
    // PNJ / objet visibles
    if (loc.npc) { const b=document.createElement("button"); b.className="exit-btn"; b.textContent="💬 Parler à "+loc.npc.name; b.onclick=()=>{this.inputEl.value="talk";this._run();}; this.exitsEl.appendChild(b); }
